...
const HomeScreen = ({ navigation }) => {
  ...
  const [checkOutLatestData, setCheckOutLatestData] = useState({});
  ...
  const handleCheckIn = async () => {
    ...
    const docRef = collection(dataBase, "userEntries", JSON.stringify(parsedData.email), "data");
    onSnapshot(docRef, (querySnapshot) => {
      ...
      data.forEach((element) => {
        if (element.date == formattedTodayDate) {
          console.log(true);
        }
      });
    ...
    try {
      const docRef = await addDoc(collection(dataBase, "userEntries", JSON.stringify(parsedData.email), "data"), {
      ...
      const docRef2 = collection(dataBase, "userEntries", JSON.stringify(parsedData.email), "data");
      onSnapshot(docRef2, (querySnapshot) => {
        let data = [];
        querySnapshot.forEach((item) => {
          if (item.data().entryData.checkOutTime == "") {
            setCheckOutLatestId(item.id);
            setCheckOutLatestData(item.data());
          }
        });
      });
  ...
  const handleCheckOut = async () => {
    ...
    try {
      const data = doc(dataBase, "userEntries", JSON.stringify(parsedData.email), "data", checkOutLatestId);

      await updateDoc(data, {
        entryData: {
          id: checkOutLatestData.entryData.id,
          checkInTime: checkOutLatestData.entryData.checkInTime,
