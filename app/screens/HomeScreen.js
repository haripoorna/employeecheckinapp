...
const HomeScreen = ({ navigation }) => {
  ...
  const handleCheckIn = async () => {
    ...
    const docRef = collection(
      dataBase,
      "userEntries",
      JSON.stringify(parsedData.email),
      formattedTodayDate
    );
    ...
    if (distance > 30) {
      Alert.alert(
        "Please come inside office , You should be in 30 meter of radius"
      );
      setIsLoading(false);
    } else {
  ...
  return (
    <View style={styles.container}>
      ...
      {isLoading ? (
        ...
        >
          <CheckInButton
            ...
            disabled={statusOfCheckInBtn}
          />

          <CheckOutButton
            onPress={handleCheckOut}
            disabled={statusOfCheckOutBtn}
          />
        </View>
      )}
      <View
      ...
      >
        {checkInValue != "" && (
          <Text
            style={{ fontSize: 10, fontWeight: "bold", marginVertical: 10 }}
          >
            Last check-in at : {checkInValue}
          </Text>
        )}
        <View></View>
        {checkOutValue != "" && (
          <Text
            style={{ fontSize: 10, fontWeight: "bold", marginVertical: 10 }}
          >
            Last check-out at : {checkOutValue}
          </Text>
        )}
      </View>
    </View>
